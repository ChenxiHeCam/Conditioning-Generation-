...
def get_args():
    ...
    p.add_argument('--max_per_z', type=int, default=None,
                   help='cap cubes per non-primary redshift')
    p.add_argument('--primary_z', type=int, default=None,
                   help='redshift exempt from max_per_z (kept in full)')
    p.add_argument('--holdout_frac', type=float, default=0.0,
                   help='fraction per z fully held out from VAE (reserved for LDM)')
    ...
    p.add_argument('--ps_k_alpha', type=float, default=0.0,
                   help='k-weighting exponent for ps_loss (positive=boost high k)')
...
def ps_loss(recon, target, n_bins=30, k_alpha=0.0):
    """
    ...
    eps_k = 0.5 / N
    if k_alpha != 0.0:
        raw_w = (centers + eps_k).pow(k_alpha)
        weights = raw_w * n_bins / raw_w.sum()
    else:
        weights = torch.ones(n_bins, device=device)
    ...
    for i in range(n_bins):
        ...
        pr = Pk_r[:, mask].mean(-1)
        pt = Pk_t[:, mask].mean(-1)
        loss = loss + weights[i] * F.mse_loss(torch.log(pr + 1e-12), torch.log(pt + 1e-12))
        used += 1
...
def main():
    ...
    def build_split(split):
        ...
        for root in [args.data_root_ic, args.data_root_astro]:
            ds = make_dataset(root, args.patch_size, args.redshifts, split,
                              ...
                              max_per_z=args.max_per_z, primary_z=args.primary_z,
    ...
    scaler    = GradScaler(enabled=False)  # bf16 needs no loss scaling
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch in train_loader:
            ...
            with autocast(enabled=(device.type == 'cuda'), dtype=torch.bfloat16):
                ...
                l_ps    = ps_loss(recon, x, k_alpha=args.ps_k_alpha) if ps_w   > 0 else torch.tensor(0.)
