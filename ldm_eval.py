...
def generate(model, vae, items, latent_mean, latent_std, sigma_data, device,
             chunk=8, num_steps=18, cfg_ic=1.0, cfg_params=1.0, latent_shape=(8,32,32,32)):
    n = len(items)
    ...
    for s in range(0, n, chunk):
        e = min(s + chunk, n)
        z = heun_sample(model, e-s, latent_shape,
...
def eval_group(items, model, vae, latent_mean, latent_std, sigma_data, device,
    ...
    if not items:
        return None
    X, R, IC = generate(model, vae, items, latent_mean, latent_std, sigma_data, device, latent_shape=LATENT_SHAPE)
    ...
    if two_runs_for_diversity:
        _, R2, _ = generate(model, vae, items, latent_mean, latent_std, sigma_data, device, latent_shape=LATENT_SHAPE)
...
def main():
    ...
    model = LDMUNet3D(latent_ch=cfg["latent_ch"], ch_mults=(1,2), attn_levels=(0,1), ic_stem_downsamples=2).to(device)
