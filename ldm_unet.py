...
class ICStem(nn.Module):
    """Learned downsampler 64^3 -> (64/2^downsamples)^3."""
    def __init__(self, in_ch=2, mid_ch=16, out_ch=32, downsamples=1):
        super().__init__()
        layers = [nn.Conv3d(in_ch, mid_ch, 3, padding=1), nn.SiLU()]
        ch = mid_ch
...
class LDMUNet3D(nn.Module):
    def __init__(self,
        ...
        self.ic_stem = ICStem(in_ch=2, mid_ch=16, out_ch=ic_stem_out, downsamples=ic_stem_downsamples)
