...
class T21Dataset(Dataset):
    def __init__(self, data_root, patch_size=64, redshifts=None,
        ...
        train_files, val_files, test_files, holdout_files = [], [], [], []
        for z in sorted(groups):
            g = [groups[z][i] for i in rng.permutation(len(groups[z]))]
            if self.max_per_z is not None and z != self.primary_z and len(g) > self.max_per_z:
                g = g[:self.max_per_z]
            n_hold = int(len(g) * self.holdout_frac)
            holdout_files.extend(g[:n_hold])
            gr = g[n_hold:]
            n_test = max(1, int(len(gr) * test_frac))
            n_val  = max(1, int(len(gr) * val_frac))
            test_files.extend(gr[:n_test])
            val_files.extend(gr[n_test:n_test + n_val])
            train_files.extend(gr[n_test + n_val:])

        self.files = {'train': train_files, 'val': val_files,
                      'test': test_files, 'holdout': holdout_files}[split]
